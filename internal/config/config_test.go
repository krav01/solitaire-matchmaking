...
func TestLoad(t *testing.T) {
	...
	base := map[string]string{
		...
		"OUTBOX_DELIVERY_URL":   "https://backend.example.com/events",
		"OUTBOX_DELIVERY_TOKEN": strings.Repeat("y", 32),
	}
	tests := []struct {
		name    string
		...
		{name: "match lease does not exceed database timeout", values: map[string]string{"MATCH_WORKER_LEASE": "5s"}, wantErr: true},
		{name: "rating lease does not exceed database timeout", values: map[string]string{"RATING_WORKER_LEASE": "5s"}, wantErr: true},
		{name: "outbox lease does not exceed database timeout", values: map[string]string{"OUTBOX_WORKER_LEASE": "5s", "OUTBOX_REQUEST_TIMEOUT": "4s"}, wantErr: true},
