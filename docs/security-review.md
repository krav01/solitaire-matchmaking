...
| Outbound delivery | Pass | Remote clear-text HTTP and redirects are rejected; requests are authenticated, timeout-bounded, lease-fenced, retried, and idempotency-keyed; the external canary verifies identical replay without a second receiver side effect |
